...
u8 TP_Read_XY(u16 *x,u16 *y)
{
	...
	xtemp=map(xtemp,EVDEV_HOR_MIN,EVDEV_HOR_MAX,0,240);
	ytemp=map(ytemp,EVDEV_VER_MIN,EVDEV_VER_MAX,0,320);
...
void TP_Save_Adjdata(void)
{
	...
	save_xoff = tp_dev.xoff;
	save_yoff = tp_dev.yoff;
	
	//保存触屏类型
	//AT24CXX_WriteOneByte(SAVE_ADDR_BASE+12,tp_dev.touchtype);	
	touchtype = tp_dev.touchtype;
...
{					  
	...
	{    												 
		...
		tp_dev.xoff = save_xoff;
 	    //得到y偏移量
		//tp_dev.yoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);	
		tp_dev.yoff = save_yoff;

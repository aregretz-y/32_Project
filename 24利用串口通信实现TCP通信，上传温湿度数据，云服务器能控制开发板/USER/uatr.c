...
__IO uint8_t  ledinitflag = 0;						 //订阅LED完成的标志
__IO uint8_t  sg90initflag = 0;						 //订阅SG90完成的标志    
__IO uint8_t dht11initflag = 0;						 //订阅DHT11完成的标志
...
void USART3_IRQHandler(void)
{
  ...
  if (USART_GetITStatus(USART3, USART_IT_RXNE) == SET)
  {
		if(u3_recvcnt < BUFSIZE)
		{
			...
			if(u3_recvbuf[u3_recvcnt-1] == '\n' 
					&& u3_recvbuf[u3_recvcnt-2] == '\r' 
					&& ledinitflag == 1 
					&& sg90initflag == 1 
					&& dht11initflag == 1)
			{

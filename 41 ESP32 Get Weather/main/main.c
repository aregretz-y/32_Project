...
* @attention
*       1.使用心智天气获取实时天气
*       2.私钥：https://api.seniverse.com/v3/weather/now.json?key=SoM-4xveIcNU20P-x&location=guangzhou&language=zh-Hans&unit=c
*       3.密钥：SoM-4xveIcNU20P-x
*
...
#define BEMFA_TOPIC "led001"


i2c_obj_t i2c0_master;
/* 链接wifi名称 */
#define DEFAULT_SSID        "cb"
/* wifi密码 */
...
void app_main(void)
{
    ...
    dest_addr.sin_addr.s_addr = inet_addr("116.62.81.138");

    //4.建立连接
    ret = connect(socket_fd,(struct sockaddr *)&dest_addr,sizeof(dest_addr));
    if(ret == -1)
    {
    ...
    //5.获取天气请求
    char sendbuf[512] = {0};
    sprintf(sendbuf,"GET /v3/weather/now.json?key=SoM-4xveIcNU20P-x&location=guangzhou&language=zh-Hans&unit=c HTTP/1.1\r\n"
    ...
    //6.接收返回值
    // 申请堆空间
    char * recvbuf = (char *)pvPortMalloc(1024 * sizeof(char));
    if (recvbuf == NULL) 
    {
    ...
    // 使用内存
    memset(recvbuf, 0, 1024);  // 初始化
    int received = recv(socket_fd, recvbuf, 1024, 0);
